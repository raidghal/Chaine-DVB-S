...
octets_rs_entrelace = convintrlv(octets_rs, 8, 3); % Entrelacement des octets RS
bits_rs_e = octettobits(octets_rs_entrelace);
%bits en octets
...
code_rs_conv_entrelace = convenc(bits_rs_e, trellis, puncmat);
...
for i = 1:length(EbN0)
    ...
    % Désentrelacement
    octets_rs = bitstooctets(decodedsoft_rs);
    octets_desentrelace = convdeintrlv(octets_rs, 8, 3);
    decodedsoft_rs_desentrelace = circshift(octets_desentrelace, -8 * 3 * 7);
    bits_rs_e = octettobits(decodedsoft_rs_desentrelace);


    % Décodage RS
    decoder_rs = comm.RSDecoder(N, K, BitInput = true);
    decodedsoft_rs_final = step(decoder_rs, bits_rs_e.');
